...
def save_mail(mail: List[Dict[str, str]]) -> None:
    """
    saves mail to a JSON file in current directory
...
def add_mail(mail_entry: Dict[str, str]) -> str:
    """
    Parameters: dictionart, mail entry
    
    Loads the json file containing all the mail entries
    Adds a mail entry to the JSON file of all mail entries
...
def delete_mail(mail_id: str) -> bool:
    """
    Parameters: string, Mail ID
    
    Loads the json file containing all the mail entries
    Remeoves the element containing the Mail ID
    
    Returns:
        returns a bool TRUE if mail was deleted else false
    """
    mail = load_mail()
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    ...
    Returns:
        returns the dictonary containing the mail I
    """
    mail = load_mail()
    for entry in mail:
        if entry['id'] == mail_id:
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """
    Parameters: string, recipiant
    
    Loads the json file containing all the mail entries
    
    Returns:
        returns a list containg all mail entry for a particular recipient
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    ...
    Returns:
        returns a list containg all mail entries for a particular sender
    """
    mail = load_mail()
    sent = []
    for entry in mail:
